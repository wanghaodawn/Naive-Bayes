...
def main():
	...
	try:
		...
		q = -1.0
	if q == -1.0:
		print("ERROR: Wrong number for q!")
		exit(-1)
	...
	non_con = math.log(q) - math.log(N_con_words + Vocabulary)
	for key in dic_vocabulary:
		if dic_con_train.has_key(key):
			dic_con_train[key] = math.log(dic_con_train[key] + q) - math.log(N_con_words + q * Vocabulary)
		else:
			dic_con_train[key] = non_con
		# print(key + ": " + str(dic_new_con[key]))
	non_lib = math.log(q) - math.log(N_lib_words + Vocabulary)
	for key in dic_vocabulary:
		if dic_lib_train.has_key(key):
			dic_lib_train[key] = math.log(dic_lib_train[key] + q) - math.log(N_lib_words + q * Vocabulary)
		else:
