...
def main():
	...
	dic_con_new = {}
	for key in dic_con_train:
		dic_con_new[key] = dic_con_train[key] - dic_lib_train[key]
		dic_lib_new[key] = dic_lib_train[key] - dic_con_train[key]

	# Get the top 20 words and print
	i = 0
	for key in sorted(dic_lib_new, key = dic_lib_new.get, reverse = True):
		if i < 20:
			temp = dic_lib_new[key]
			temp = round(temp, 4)
			print(key + " " + str(temp))
			i += 1
		else:
	...
	i = 0
	for key in sorted(dic_con_new, key = dic_con_new.get, reverse = True):
		if i < 20:
			temp = dic_con_new[key]
			temp = round(temp, 4)
			print(key + " " + str(temp))
			i += 1
		else:
